...
{
    public static class AlzadoReconstructor
    {
        /// <summary>
        ...
        private static List<double[]> ExtraerVertices(List<ComponenteLong> componentes)
        {
            ...
            for (int i = 1; i < componentes.Count; i += 2)
            {
                    ...
                    componentes[i].getTipoComponente() == ComponenteLong.tipoComponente.parabola)
                {
                    ...
                    // El vértice está en el punto medio del acuerdo parabólico
                    // Calculamos el PK y la cota del vértice teórico
                    double pkEntrada = parabola.getPkIni();
                    double longitudParabola = parabola.getLongutid();
                    double pkVertice = pkEntrada + longitudParabola / 2.0;

                    // La cota del vértice se calcula en el centro de la parábola
                    double xVertice = parabola.getPuntoEntrada.coordenadaX +
                                     (parabola.getPuntoSalida.coordenadaX -
                                      parabola.getPuntoEntrada.coordenadaX) / 2.0;

                    double yVertice = parabola.getPuntoEntrada.coordenadaY +
                                     (parabola.getPuntoSalida.coordenadaY -
                                      parabola.getPuntoEntrada.coordenadaY) / 2.0;
