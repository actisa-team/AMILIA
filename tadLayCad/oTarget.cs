...
namespace tadLayCad
{
    ...
    public class oTarget
    {
        ...
        #region "Constructor"
        public oTarget(oP2d iPtoIni, oP2d iPtoFin)
        {
        ...
        /// <summary>
        /// Obtener Puntos Target definidos por el usuario
        /// </summary>
        /// <param name="iLayerPointsTarget">Capa Puntos</param>
        /// <param name="AddPtoIniFin">Añadir Pto Inicial y Final</param>
        /// <param name="iFiltro">Filtro Datos</param>
        public void setListPtoTargetFromLayer(string iLayerPointsTarget,bool AddPtoIniFin, eFiltro iFiltro)
        {

            List<oP2d> myLstTargetScw = new List<oP2d>();
            
            SelectionSet mySsPtoTarget = engCadNet.oSs.getSsByLayerAndEntidad(iLayerPointsTarget, engCadNet.eEntidades.POINT);

            if (mySsPtoTarget != null && mySsPtoTarget.Count > 0)
            {
                ...
                using (DocumentLock myDockLock = oCadManager.thisEditor.Document.LockDocument())
                {
                    using (Transaction tr = oCadManager.StartTransaction())
                    {
                        foreach (ObjectId myObj in mySsPtoTarget.GetObjectIds())
                        {
                            DBPoint myPto = myObj.GetObject(OpenMode.ForRead) as DBPoint;

                            myLstTargetScw.Add(new oP2d(myPto.Position.X, myPto.Position.Y));
            ...
            //Añado el Punto Inicial y Final a la Coleccion

            if (AddPtoIniFin)
            {   
                myLstTargetScw.Add(new oP2d(mPtoIni.X,mPtoIni.Y));
                myLstTargetScw.Add(new oP2d(mPtoFin.X,mPtoFin.Y));          
            }
